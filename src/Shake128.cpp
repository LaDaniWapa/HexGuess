...
void Keccakp(int nr, vector<uint8_t>& S)
{
    int l = 6;
    uint64_t A[25] = {0};
    for (int i = 0; i < 25; i++)
    {
        for (int j = 0; j < 8; j++)
        {
            A[i] += static_cast<u_int64_t>(S[8 * i + j]) << (8 * j);
        }
    }
...
void pad0star_1(int x, int m, vector<uint8_t>& out)
{
...
void sponge(vector<uint8_t>& N, int d_in_bytes, int r, vector<uint8_t>& out)
{
    ...
    vector<uint8_t> rep(Z.begin(), Z.begin() + d_in_bytes);
    out.clear();
    out = rep;
}


void keccak(int c, vector<uint8_t>& S, int d, vector<uint8_t>& out)
{
